          ...
          <div className={`${mobileTab === 'chat' ? 'flex' : 'hidden'} ${isPdfMaximized ? 'lg:hidden' : 'lg:flex lg:col-span-4 xl:col-span-3'} flex-col bg-white min-h-0 overflow-hidden`}>
            ...
            <div className="p-3 border-b border-slate-200 bg-slate-50 grid grid-cols-3 gap-2 shrink-0">
              <button
                ...
                className="p-2 rounded-xl bg-white hover:bg-slate-100 border border-slate-200 text-left transition-all group shadow-2xs"
              >
                <div className="text-[10px] text-slate-500 font-semibold flex items-center justify-between">
                  <span>FRI Score</span>
                  <ArrowUpRight className="w-2.5 h-2.5 text-slate-400 group-hover:text-blue-900" />
                </div>
              ...
              >
                ...
                <div className="text-xs font-black text-blue-900 mt-0.5">RM {safeMaxInstallment.toLocaleString('en-MY')}</div>
              </button>
            ...
            <div className="px-3 py-2 border-b border-slate-200 bg-white flex items-center gap-1.5 overflow-x-auto scrollbar-none shrink-0">
              {quickActionChips.map((chip, idx) => (
                <button
                  key={idx}
                  onClick={() => handleSendMessage(chip.prompt)}
                  className="px-2.5 py-1 rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 text-[11px] font-bold whitespace-nowrap border border-slate-200 transition-all flex items-center gap-1 shrink-0 cursor-pointer active:scale-95 shadow-2xs"
                >
