    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-blue-950/70 backdrop-blur-xs p-4 animate-fade-in">
      <div className="w-full max-w-md bg-white rounded-3xl shadow-2xl overflow-hidden border border-slate-200 flex flex-col">
        ...
        <div className="p-7 flex flex-col gap-5">
          ...
          {/* Quick Account Switcher for Testing (Guest vs Pro) */}
          <div className="flex flex-col gap-2 p-3 bg-slate-50 border border-slate-200 rounded-2xl">
            <div className="flex items-center justify-between">
              <span className="text-[10.5px] font-bold uppercase tracking-wider text-slate-500">
                {language === 'bm' ? 'Pilih Akaun Ujian Segera' : 'Quick Test Accounts'}
              </span>
              <span className="text-[9.5px] font-semibold text-blue-900 bg-blue-100/70 px-1.5 py-0.5 rounded">
                1-Click Login
              </span>
            </div>

            <div className="grid grid-cols-2 gap-2">
              {/* 1. Guest Tester Account (Free Preview Tier - Always Locked on Login) */}
              <button
                type="button"
                onClick={() => {
                  try {
                ...
                className="p-2.5 bg-white hover:bg-slate-100/80 border border-slate-200 hover:border-slate-300 rounded-xl text-left transition-all flex flex-col gap-1 cursor-pointer group shadow-2xs"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-900 group-hover:text-blue-950">
                    🟢 {language === 'bm' ? 'Akaun Guest' : 'Guest Account'}
                  </span>
                ...
                <p className="text-[10px] text-slate-500 leading-tight">
                  {language === 'bm' ? 'Percuma & Uji Paywall (Sentiasa Reset)' : 'Free preview & test paywall (Always resets)'}
                </p>
              </button>

              {/* 2. Premium Pro Account (1-Month Paid Subscriber - Permanently Unlocked) */}
              <button
                type="button"
                onClick={() => {
                  try {
                ...
                className="p-2.5 bg-[#091E42] hover:bg-[#071735] text-white rounded-xl text-left transition-all flex flex-col gap-1 cursor-pointer shadow-2xs"
              >
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-white">
                    💎 {language === 'bm' ? 'Akaun Pro' : 'Premium Pro'}
                  </span>
                  <span className="text-[9px] font-bold bg-amber-400/20 text-amber-300 border border-amber-400/30 px-1.5 py-0.5 rounded">
                    1 Month
                  </span>
                </div>
                <p className="text-[10px] text-slate-300 leading-tight">
                  {language === 'bm' ? 'Langganan RM 19.90 (Dibuka Sepenuhnya)' : 'Paid RM 19.90 (Fully Unlocked)'}
                </p>
              </button>
            </div>
          </div>
          ...
          <div className="relative flex items-center justify-center">
            ...
            <span className="bg-white px-2 text-[10px] uppercase font-bold text-slate-400 absolute">
              {language === 'bm' ? 'atau log masuk manual' : 'or manual login'}
            </span>
          </div>
